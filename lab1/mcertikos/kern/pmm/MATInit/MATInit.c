...
void pmem_init(unsigned int mbi_addr)
{
    ...
    table_row_num=get_size();

    if(table_row_num>0){
        start_address=get_mms(table_row_num-1);
        length=get_mml(table_row_num-1);
        highest_address=start_address+length-1;
    ...
    for (unsigned int i=0;i<nps;i++){
        if(i<VM_USERLO_PI || i>=VM_USERHI_PI){
            ...
            unsigned int flag=0;
            for(unsigned int j=0;j<table_row_num;j++){
                start_address=get_mms(j);
                length=get_mml(j);
                end_address=start_address+length-1;
                if(i>=start_address/PAGESIZE && i<=end_address/PAGESIZE){
                    flag=1;
                    break;
                }
            }
